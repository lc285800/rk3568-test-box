...
7. 勾选“确认写入”。
8. 点击“输出”。
9. 观察 LED、万用表或被测板卡输入状态。
10. 查看实时日志和任务结果。

注意：当前输出使用 `gpioset --mode=time`，只保持指定时长。时间结束后，GPIO 可能回到默认状态。
...
这是当前设计。GPIO 输出使用短时模式：

```bash
gpioset --mode=time
```

时间结束后 GPIO 可能恢复默认状态。这个行为更安全，适合作为测试盒子的第一版输出方式。
