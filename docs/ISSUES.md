...
## 2026-05-14 UART 读取语义不符合串口助手习惯

阶段/外设：UART 实测，Web 控制台和 UART Adapter。

现象：

- Web 点击“发送并读取”，PC 串口助手可以收到 Web 发送的数据。
- 先从 PC 串口助手发送 `12345`，再点击 Web“读取”，Web 结果为空。
- “发送并读取”这个按钮名称容易让人误解为普通串口助手的持续接收。

原因：

- 原“读取”是一次性打开串口、等待约 500ms、读取一次后关闭，不是持续监听。
- 如果 PC 端先发，再点击 Web 读取，数据可能已经错过 Web 端打开串口的接收窗口。
- “发送并读取”实际设计用于命令-响应场景，例如 AT 指令、查询命令：Web 发送命令后短时间等待对方立即回包。

修复：

- 新增 UART `listen` 动作，默认打开约 5 秒接收窗口。
- Web 按钮调整为：
  - “监听接收”：用于先打开 Web 接收窗口，再从 PC 串口助手发送数据。
  - “发送”：只发送，不等待响应。
  - “发送并等回复”：用于命令-响应场景。
- 更新 QUICKSTART 和 PROJECT_STATUS，明确 UART 接收测试顺序。

验证：

```text
本地自动化测试：pytest -> 21 passed
板端自动化测试：pytest -> 21 passed
用户实测：UART 基本功能正常，Web 发送到 PC 串口助手、接收测试主链路可用。
```

状态：已修复，UART 基本功能已通过用户实测。

## 2026-05-13 UART 面板按钮溢出右侧

阶段/外设：UART 实测准备，Web 控制台。

现象：

- Safari 打开 `http://192.168.2.88:8080` 后，UART 测试面板把端口、波特率、校验、数据、换行、读取字节、Dry run、确认发送和三个按钮全部排在同一行。
- “发送并读取”按钮超出面板右侧，被浏览器窗口裁切。

原因：

- `.uart-form` 使用 9 个固定网格列。
- 按钮区域没有单独换行，宽度不足时仍尝试塞进同一行。

修复：

- 将 UART 表单调整为最多 6 列输入区。
- Dry run 和确认发送放到下一行。
- 操作按钮独立占满一行，并允许按钮行换行。
- 更新静态资源版本为 `20260513-uart-layout`，避免 Safari 缓存旧 CSS。

验证：

```text
Safari / Computer Use -> http://192.168.2.88:8080
刷新页面后，UART 输入、确认项、按钮分行显示。
点击 UART “列举” -> 任务 completed，结果区域正常显示，按钮不再溢出右侧。
```
